...
//                BrainyFilter.sendRequest();
        ...
        prepareFilterData: function(full, source) {
            var path, query;
            var nRoute = '';
            var nPath  = '';
            var str = window.location.search;
            if (str.match(/[\?\&]source\=/)) {
                var nRoute = str.replace(/(.*)([\?\&])(source\=)([^\&]+)(.*)/, '$4');
            }
            if (str.match(/[?\&]path\=/)) {
                var nPath  = str.replace(/(.*)([\?\&])(path\=)([^\&]+)(.*)/, '$4');
            }
            path  = window.location.pathname;
            query = window.location.search.replace(/[\&\?]+((source)|(path)|(page)|(bfilter)|(search)|(category_id)|(sub_category)|(description)|(manufacturer_id))[^&]+/g, '');
            ...
            if (source) {
                form = form.replace(/[\&](source)=[^\&]+/g, '').replace(/[\&]+$/, '');
                form = 'source=' + source + form;
            } else {
                form = form.replace(/[\&](source|path|manufacturer_id)=[^\&]+/g, '').replace(/[\&]+$/, '');
                if (nPath !== '') {
                    form = 'path=' + nPath + form;
                }
                if (nRoute !== '') {
                    form = 'source=' + nRoute + '&' + form;
                }
            }

            var str= window.location.toString();
            str = str.match(/[\?\&]source\=/);
            if (full === true && !str) {
                form = form.replace(/[\&\?]((source)|(path))[^&]+/g, '');
            }
